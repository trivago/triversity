    ...
    <div class="detail__project__title">
      ...
      <div class="button-group">
        <b-button
          ...
          v-b-tooltip.hover
          title="Edit"
          @click="$router.push({ name: 'EditProjectPage', params: { recordId: recordId} })"
        >
        ...
        <md-card class="card--white flex--weight-2">
          ...
          <div class="card__content" v-if="universityIds">
            <div class="card--shadow" v-for="university in universities" :key="university.UniCode">
              <div
                class="card--shadow__content card--shadow__content--image-bg"
              ...
              >
                ...
                <span class="card--shadow__description">{{ university.Address }}</span>
                <br />
  ...
  methods: {
    ...
    async getTables(tableName, idArray, dataArray) {
      for (let i = 0; i < idArray.length; i++) {
        let response = await VueAirtableService.getRecord(
          tableName,
          idArray[i]
        );
        if (response.status === 200) {
    ...
    onDelete: function(id) {
      if (confirm("Do you really want to delete?")) {
        VueAirtableService.deleteRecord("Project", id)
          .then(res => {
