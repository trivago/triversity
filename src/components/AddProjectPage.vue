  ...
  <div class="create-form--container">
    ...
    <b-form @submit="onSubmit" v-if="show" class="project-form">
      ...
      >
        ...
        <md-chip
          md-deletable
          @md-delete="onDeleteAttachment(file)"
          v-for="file in form.attachment"
          :key="file.id"
        >{{ file.filename }}</md-chip>
      </b-form-group>
      <div class="div-buttons">
        ...
        <b-button
          ...
          type="cancel"
          variant="danger"
          @click="$router.push({name: 'ProjectListView'})"
        >Cancel</b-button>
      </div>
    </b-form>
  </div>
...
export default {
  ...
  methods: {
    async fillInputForm(recordId) {
      var response = await VueAirtableService.getRecord(
        "Project",
        recordId
      ).catch(e => {
        console.log("Error: " + e);
      ...
      if (response.status === 200) {
        ...
        this.form.projectDescription =
          response.data.fields["Project Description"];
    ...
    async onSubmit(evt) {
      ...
      if (this.recordId != null) {
        response = await VueAirtableService.updateRecord(
          "Project",
          this.recordId,
          data
        );
    ...
    async addUniversity(uniName) {
      ...
      var createUniResponse = await VueAirtableService.createRecord(
        "University",
        data
      );
    ...
    async addTargetGroup(targetGroupName) {
      ...
      var createTgResponse = await VueAirtableService.createRecord(
        "TargetGroup",
        data
      );
    ...
    async addMentor(mentorName) {
      ...
      var createMentorResponse = await VueAirtableService.createRecord(
        "Mentor",
        data
      );
    ...
    callFilestackApi() {
      const client = require("filestack-js").init(
        process.env.VUE_APP_FILESTACK_API_KEY
      );
      let options = {
        maxFiles: 5,
        accept: [
