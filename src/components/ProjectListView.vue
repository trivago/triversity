  ...
  <div class="listview-content">
    ...
    <b-container fluid class="filter-container">
      <b-row>
        ...
        <b-col>
          <FilterMultiselect
            @messageFromFilterMultiselect="childMessageReceived"
            :field="'tgTable'"
            :options="filterTargetGroupOptions"
            :multiple="true"
            :taggable="true"
            ref="filterMultiSelect"
          />
        </b-col>
        <b-col>
          <FilterMultiselect
          ...
          <md-list-item md-expand :md-expanded.sync="record._showDetails">
            ...
            <div slot="md-expand">
              <ProjectListExpandPreview
                :record="record"
                @messageFromProjectListExpandPreview="childMessageReceived"
              ></ProjectListExpandPreview>
            </div>
          </md-list-item>
        </div>
  ...
  mounted: function() {
    if (!this.base) {
      return console.error(
        "Please specify `base` and `columns` attributes for <vue-airtable> component."
      );
    }
    this.getData();
    this.getTargetGroup();
  ...
  methods: {
    async getData() {
      ...
      var sort =
        ...
        this.sort.direction;
      this.isLoading = true;
      var response = await VueAirtableService.getRecords(
        "Project",
        joinQueryForAllFilters,
        sort
      );
