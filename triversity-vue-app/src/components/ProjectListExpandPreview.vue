  ...
  <div class="project__preview flex-direction--column">
    <div class="flex-direction--row">
      ...
      <div class="flex-direction--column flex--weight-1">
        <md-card class="card--white flex--weight-1">
          <small>
            <md-icon v-if="record.fields['TargetGroups']" class="icon--size-sm color--trivago-blue">fiber_manual_record</md-icon>
        ...
        <md-card class="card--white flex--weight-2">
          <small>
            <md-icon v-if="record.fields['Mentors']" class="icon--size-sm color--trivago-orange">fiber_manual_record</md-icon>
        ...
        <md-card class="card--white flex--weight-2">
          <small>
            <md-icon v-if="record.fields['Universities']" class="icon--size-sm color--trivago-red">fiber_manual_record</md-icon>
    ...
    <div class="button-group">
      <b-button variant="link" class="text-decoration-none"
                ...
                @click="sendButtonEventToParent(record.id)"><md-icon>delete</md-icon></b-button>
      <b-button variant="link" class="text-decoration-none"
                v-b-tooltip.hover title="Edit"
                @click="$router.push({ name: 'EditProjectPage', params: { recordId: record.id} })"><md-icon>edit</md-icon></b-button>
      <b-button variant="link" class="text-decoration-none"
                v-b-tooltip.hover title="Show details"
                @click="$router.push({ name: 'ProjectDetailPage', params: { recordId: record.id} })"><md-icon>visibility</md-icon></b-button>
    </div>
  </div>
</template>
